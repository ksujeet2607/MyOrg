...
class Admin extends Session{
    ...
    public function logout($param) {
        ...
        auth::DestroySess('adminuser');
        $this->redirect(BASE_URL."login", "Logged out!!~suc");
    }

    public function adminhome($param){
        $rs = $this->db_executeSingle("news", "count(*)", "status=0");
        $rs1 = $this->db_executeSingle("feedback", "count(*)", "readstatus=0");
        $rs2 = $this->db_executeSingle("profile", "count(*)", "status=0");
        $this->render("../view/admin/adminhome.php", array("list",$rs,$rs1,$rs2));
    }
  ...
  //********************Add/Update Profile*************************//
    ...
    public function searchprofile($param){
        //print_r($param);
        $this->isAuthAdmin("manageProfile", "Show");
            if(isset($_POST['search']) || isset($param[0])){
                if(isset($param[0])){
                   $_POST['SearchProfile'] = $this->strip($param[0]);
                   $_POST['SearchCriteria'] = "All";
                   $_POST['SearchValue'] = "";
                }
                $SearchProfile = $_POST['SearchProfile'];
                $SearchCriteria = $_POST['SearchCriteria'];
                ...
                if($this->is_not_empty($_POST,"SearchProfile~@~SearchCriteria~@~SearchValue")){
                    ...
                    $rs_search = $this->db_executeReader("profile", "profile.*, (CASE WHEN (reg_by = 0) THEN 'Self' ELSE l. displayname END) as addedby", " LEFT JOIN Login l on l.id = reg_by", $cond," order by id desc",false);
    ...
    public function addbusiness($param){
         $this->render("../view/admin/addBusiness.php", $param);
    }

    public function addProduct(){
        $qnt = $this->strip($_POST['quantity']);
        $selProfileID = $this->strip($_POST['selProfileID']);
        $sql =" INSERT INTO proSer(profileid,name,type,descrp,reg_by,entrydate,imgurls) VALUES";

        for($i =0; $i < $qnt; $i++){
            $btype = $_POST['businessType'][$i];
            $productDesc = $_POST['productDesc'][$i];
            $pname = $this->ucf($this->strip($_POST['product'][$i]));
            $olddata = $_POST['olddata'][$i];
            $photo = "photo".$i;
            if($olddata!=""){
                 $$photo = $this->db_executeSingle("proSer", "imgurls", "id=$olddata");
                 $$photo .="||";
            }else{
                 $$photo = "";
            }
...
//            print_r($_POST['newImgs']);
//            die();
            for($j=0;$j<count($_FILES['productImg']['name'][$i]);$j++){
                if($_FILES['productImg']['name'][$i][$j]!="" && in_array($_FILES['productImg']['name'][$i][$j], $_POST['newImgs'][$i])){
                    $uploadfile=$_FILES["productImg"]["tmp_name"][$i][$j];
                    $img_name = $_FILES['productImg']['name'][$i][$j];
                    ...
                    $$photo .= $resized."||";
                    unlink(__DIR__."/../assets/products/".$new_name);
                    header('Content-Type: text/html');
                    $len = strlen(BASE_URL);
...
//            die();
            $imgurls = rtrim($$photo,"||");
            if(!empty($btype)){
              $sql .= "('$selProfileID','$pname','$btype','$productDesc',".$this->userid.",'".date("Y-m-d H:i:s")."','$imgurls'),";
            }
        }
        $this->db_execute("DELETE FROM proSer where profileid='$selProfileID'",false);
        $sql = rtrim($sql,",");
        $this->db_execute($sql,false);
        $msg = "Business Details Added.~suc";
//        die();
        $this->redirect (ADMIN_URL."addbusiness",$msg);
    }

    public function deleteBusinessImg(){

        $imgpath = $this->db_executeSingle("proSer", "imgurls","id='".$this->strip($_POST['id'])."'","",false);
        $url = $_POST['src'];
        $len = strlen(PUBLIC_URL);
        $new_path = substr($url, $len, strlen($url)-$len);
        $new_path = __DIR__."/../".$new_path;
        ...
        $this->db_Update("proSer", "imgurls", $imgpath, "id='".$this->strip($_POST['id'])."'", false);
    ...
    public function removebusiness(){
        ...
        if($this->db_delete("proSer", "id='".$this->strip($_POST['id'])."'", false)){
           echo '1';
        }else{
           echo '2';
        }
        ...
        $daughterunmarried = $this->strip($_POST['daughterunmarried']);
           ...
           $educ = $this->db_executeSingle("education", "Education","Eduid='".$this->strip($_POST['educ'])."'");
            foreach ($_FILES as $key=>$value) {
                ...
                unlink(__DIR__."/../assets/member/".$new_name);
                header('Content-Type: text/html');
                $len = strlen(BASE_URL);
                $new_path = substr($photo, $len, strlen($photo)-$len);
                $new_path = __DIR__."/../".$new_path;
                $filename = "thumb_".basename($new_path);
                $thumb = __DIR__."/../assets/member/".$filename;
                if(file_exists($new_path)){
                   unlink($new_path);
                   unlink($thumb);
                }
...
//**************** newscat Master ****************//
    ...
    public function managenewscat($param) {
        $this->isAuthAdmin(__FUNCTION__, "Show", "managenewscat");
        $this->render("../view/admin/managenewscat.php", $param);
    }
            // //


    // LIST News Cat FROM AJAX INTO DATATABLES //
    public function getnewscatRecord($param){
        $search = $_POST['search']['value'];
        $cond = ($search!="")?" (newscatName like '".$search."%')":"1=1";
        $limit = $param[0];
        $cpage = $param[1];
        if($cpage==1){
            $start = 0;
        ...
        $lim = ($cond!="1=1")?"":$lim;
        $draw = $_POST['draw'];$result = array();
        $rs = $this->db_execute("select * from newscat where $cond order by newscatName $lim", false);
        $count = ($cpage==0)?0:($cpage-1)*$limit;
        while ($row = $this->db_read($rs)){
            $action = '<button class="btn btn-flat btn-warning" onclick="window.location.href=\''.ADMIN_URL.'editnewscat/'.$row['Id'].'\'">
                        Edit
                       </button> |
                       <button class="btn btn-flat btn-danger" onclick="deletenewscat('.$row['Id'].')">
                           Delete
                        </button>';
            $data = array("DT_RowId" => "row_".$row['Id'],++$count,"".$row['newscatName']."",$action);
            array_push($result, $data);
        }
        $response = array( "draw" => $draw );
        $response['data'] = $result;
    ...
    // ADD News Cat  //
    public function addnewscat($param) {
        $this->isAuthAdmin("managenewscat", "Add");
        extract($_POST);
        $cnt = $this->db_executeSingle("newscat", "COUNT(*)","newscatName='".$this->strip($newscatname)."'","",FALSE);
        if($cnt > 0){
            $msg = "Duplicate News Category name.~err";
        }else{
            $this->db_Insert("newscat","newscatName,user_id,entrytime", $this->ucf($this->strip($newscatname))."~".auth::Get_userID().
                    "~". date("Y-m-d H:i:s"), false);
            $msg = "News Category Added.~suc";
        }
        $this->redirect (ADMIN_URL."managenewscat",$msg);
    }
    // //

    // SHOW News Cat DETAILS FOR EDIT //
    public function editnewscat($param) {
        $this->isAuthAdmin("managenewscat", "Edit");
        if(isset($param[0]) || count($param[0])!=0){
            $newscatName = $this->db_executeSingle("newscat", "newscatName","Id='".$this->strip($param[0])."'");
            $response = array("edit",$newscatName,$param[0]);
            $this->managenewscat($response);
        }else{
          $this->redirect (ADMIN_URL."managenewscat","Required data missing.~err");
        }
    }
    // //
    ...
    public function updatenewscat(){
        $this->isAuthAdmin("managenewscat", "Edit");
        extract($_POST);
        $cnt = $this->db_executeSingle("newscat", "COUNT(*)","newscatName='".$this->ucf($this->strip($newscatname))."' AND Id!=".$this->strip($_POST['id']));
        if($cnt > 0){
            $msg = "Duplicate newscat name.~err";
        }else{
            $this->db_Update("newscat","newscatName", $this->ucf($this->strip($newscatname)),"Id=".$this->strip($_POST['id'])."",false);
    ...
    // DELETE News Cat //
    public function deletenewscat($param) {
        $this->isAuthAdmin("managenewscat", "Delete");
        if(isset($param[0]) || count($param[0])!=0){
            $this->db_delete("newscat", "Id='".$this->strip($param[0])."'",false);
                echo "1";
            }
        else{
            echo "2";
   ...
   // Add NEWS//
    ...
    public function addnews($param,$update=false,$img_old=array()) {
        ...
        if($_FILES['news_head_img']['name']!=""){
            ...
            unlink(__DIR__."/../assets/newsimages/".$new_name);
            header('Content-Type: text/html');
            $len = strlen(BASE_URL);
            $new_path = substr($photo, $len, strlen($photo)-$len);
            $new_path = __DIR__."/../".$new_path;
            $filename = "thumb_".basename($new_path);
            $thumb = __DIR__."/../assets/newsimages/".$filename;
            if(file_exists($new_path)){
               unlink($new_path);
               unlink($thumb);
            }
...
//        die();
        ...
        $content_title = $this->strip($_POST['content_title']);
        $newsSTR = Strip_tags($_POST['newsdetails']);
        if($content_title=="")
        $content_title = substr($newsSTR, 0, 170);

        $this->db_Insert("news", "title,content_title,header_image,used_images,video_link,news_date,"
                . "show_as,news_content,newsstr,entrydate,image_caption,added_by",
        $this->strip($_POST['news_head'])."~".$content_title."~".
                $this->strip($head_img_new)."~".$this->strip($string)."~".$this->strip($_POST['video_link'])."~".
                $this->formatdate($_POST['news_date'])."~".
                $this->strip($show_as)."~".$this->strip($_POST['newsdetails'])."~".$newsSTR."~".
...
//        die();
    ...
    public function modifynews($param) {
        ...
        $response = array("list",$date1,$date2,$status,$addedby,$param[0]);
        $this->render("../view/admin/modifynews.php", $response);
    }
    // //


// SHOW NEWS LIST ON feedback //
    public function feedback($param) {
        if(isset($param[0])){
            $status = $param[0];
            $cond = "readstatus = ".$this->strip($param[0]);
        }
        $limit = 50;
        $cpage = (isset($_GET['page']))?$_GET['page']:1;
        $pagi =  $this->pagination("feedback",$limit,$cpage);
        if($cpage==1){
            $start = 0;
           }else{
               $start = ($cpage * $limit) - $limit;
        ...
        $lim = " limit $start ,$limit";
        $rs = $this->db_executeReader("feedback", "*", "", $cond,"order by entrydate DESC $lim");
        $pagi = $this->pagination("feedback WHERE $cond",$limit,$cpage);
        $response = array("list",$rs,$pagi,$cpage,$limit);
        $this->render("../view/admin/feedback.php", $response);
    }

    public function updateFeedBacSts($param){
        $ids = $_POST['ids'];
        $this->db_Update("feedback", "readstatus", "1", " id IN ($ids)");
        echo "1";
    }
    ...
    public function deleteFeedback($param) {
        $this->isAuthAdmin("feedback", "Delete");
        if(isset($param[0]) || count($param[0])!=0){
            $this->db_delete("feedback", "id='".$this->strip($param[0])."'",false);
                echo "1";
            }
        else{
            echo "2";
...
//        die();
        ...
        $this->strip($_POST['pagename'])."~". $this->ucf($this->strip($_POST['pagetitle']))."~".
                ...
                ."~".$this->userid."~".date("Y-m-d h:i:s"),
                "pagename='".$this->strip($_POST['pagename'])."'", false);
            ...
            for($i=0;$i<count($_FILES["upload_file"]["name"]);$i++)
             {
              ...
              unlink(__DIR__."/../assets/gallery/".$new_name);
                header('Content-Type: text/html');
                $len = strlen(BASE_URL);
                $new_path = substr($photo, $len, strlen($photo)-$len);
                $new_path = __DIR__."/../".$new_path;
                $filename = "thumb_".basename($new_path);
                $thumb = __DIR__."/../assets/gallery/".$filename;
                if(file_exists($new_path)){
                   unlink($new_path);
                   unlink($thumb);
                }
...
//       die();
  ...
  //////************* Slider Image ************///////////
    ...
    public function manageslider($param) {
        $this->isAuthAdmin("manageslider", "Show");
    ...
    public function addSlider($param){
        ...
        for($i=1;$i<=$numssld;$i++){
            ...
            if($uploadfile!=""){
                ...
                $url = $this->resize_image(__DIR__."/../assets/slider/".$new_name, 960, 640,FALSE,'slider');
    ...
    public function removeSlider($param) {
        $this->isAuthAdmin("manageslider", "Delete");
        if($param[0]!=""){
